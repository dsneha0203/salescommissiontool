...
<script>
	...
	function sendData() {
		...
		window.opener.document.getElementById('managerId').value = x;
		window.close();
	}
...
#textboxid {
	...
	min-width: 120px;
		...
		<body>
		<form action="/CommissionTool/submitEmpManager" method="post">
				<div id="menu"></div>
				   ...
				   <thead>
					...
					<c:forEach items="${emp}" var="emp1">
							<tr>
								...
								<td><input type="checkbox" value="${emp1.employeeName}" id="choose" onclick="sendData();"></td>	  
							</tr>						
					</c:forEach>	
				</table> 
			</c:if>
